...
/**
 * Delete project-admin.css and project-admin.min.css before we minify and optimize
 */
gulp.task( 'clean:styles', () =>
	del( [ 'assets/css/' + packageJSON.name + '-admin.css', 'assets/css/' + packageJSON.name + '-admin.min.css', 'assets/css/' + packageJSON.name + '-front-end.css', 'assets/css/' + packageJSON.name + '-front-end.min.css' ] )
);
...
gulp.task( 'postcss', [ 'clean:styles' ], () =>
	gulp.src( 'assets/sass/*.scss', paths.css )
		...
		// Create project-admin.css.
		.pipe( gulp.dest( 'assets/css/' ) )
		.pipe( browserSync.stream() )
);

/**
 * Minify and optimize project-admin.css.
 *
 * https://www.npmjs.com/package/gulp-cssnano
 */
gulp.task( 'cssnano', [ 'postcss' ], () =>
	gulp.src( 'assets/css/' + packageJSON.name + '-admin.css' )
		.pipe( plumber( {'errorHandler': handleErrors} ) )
		.pipe( cssnano( {
			'safe': true // Use safe optimizations.
		} ) )
		.pipe( rename( packageJSON.name + '-admin.min.css' ) )
		.pipe( gulp.dest( 'assets/css' ) )
		.pipe( browserSync.stream() )
);
...
gulp.task( 'cssnano-frontend', [ 'postcss' ], () =>
	gulp.src( 'assets/css/' + packageJSON.name + '-front-end.css' )
		.pipe( plumber( {'errorHandler': handleErrors} ) )
		.pipe( cssnano( {
			'safe': true // Use safe optimizations.
		} ) )
		.pipe( rename( packageJSON.name + '-front-end.min.css' ) )
		.pipe( gulp.dest( 'assets/css' ) )
		.pipe( browserSync.stream() )
);
...
gulp.task( 'concat-admin', () =>
	gulp.src( paths.concat_scripts_admin )
		...
		.pipe( concat( packageJSON.name + '-admin.js' ) )

		// Append the sourcemap to project-admin.js.
		.pipe( sourcemaps.write() )

		// Save project-admin.js
...
gulp.task( 'concat-front-end', () =>
	gulp.src( paths.concat_scripts_front_end )
		...
		// Concatenate partials into a single script.
		.pipe( concat( packageJSON.name + '-front-end.js' ) )

		// Append the sourcemap to project-front-end.js.
		.pipe( sourcemaps.write() )

		// Save project-front-end.js
		.pipe( gulp.dest( 'assets/js' ) )
		.pipe( browserSync.stream() )
);
 ...
 */
gulp.task( 'clean:pot', () =>
	del( [ 'languages/' + packageJSON.name + '.pot' ] )
);
